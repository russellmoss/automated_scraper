...
- 🔄 **Retry Queue** - Local-first data queue with automatic retry on failures
- 🎯 **Source Mapping** - Map multiple sources to different output workbooks
- 📜 **Execution History** - Track all scraping runs with live profile count updates
- 🔗 **Clickable Links** - Direct links to scraped data in Google Sheets from execution history
...
2. Go to "🔍 Manual Scraping" section
...
7. Click "⏹ Stop" to stop scraping at any time
...
8. View execution history in "📜 Execution History" section
...
2. Path 1: Filter `category` equals `"status"` → Route to Status Channel
3. Path 2: Filter `category` equals `"error"` → Route to Error Channel
...
|------|------|-------|----------|-------------------|--------------|-------------------|
| YYYY-MM-DD | John Doe, CFP® | Financial Advisor | New York, NY | Taylor Matthews | https://... | CFA |
...
7. **Accreditation 1-6**: Professional accreditations (CFA, CFP®, MBA, etc.)
...
```
automated_scraper/
├── manifest.json              # ⚠️ DO NOT COMMIT - contains your OAuth Client ID
├── manifest-template.json     # ✅ Template for creating manifest.json
├── .gitignore                 # Excludes manifest.json and sensitive files
├── background/
│   ├── service_worker.js      # Main orchestration hub
